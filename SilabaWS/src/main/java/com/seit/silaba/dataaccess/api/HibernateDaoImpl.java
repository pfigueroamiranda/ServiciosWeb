package com.seit.silaba.dataaccess.api;
...
public class HibernateDaoImpl<T, PK extends Serializable> implements Dao<T, PK> {
    ...
    public List<T> findByParticular(String queryName, String nameParam, Object valueParam) {
        log.debug("finding " + entityClass.getName() +
            " instance with property: " + queryName + ", value: " + valueParam);
        
        try {
    		 
    		Query query = sessionFactory.getCurrentSession().getNamedQuery(nameParam);
    		query.setParameter(nameParam, valueParam);
